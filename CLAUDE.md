...
/billing                   → Billing             (ProtectedRoute — subscription management)
...
- **Free tier:** Full-page paywall — how-it-works (stacked vertical layout, not 3-col grid), use-cases grid, lock gate with upgrade CTA → opens `UpgradeModal`
- **Paid tier (Pro/Unlimited):**
  - Active webhook banner showing saved URL
  - Webhook config card: URL input + Save + Test Connection button
  - Test fires dummy payload with `proposal_id: 'test-00000000-...'` and `grand_total: 5000`
  - Status indicators (saved / error / webhook-ok / webhook-error) with `AnimatePresence`
  - Platform guides accordion: Make.com, Zapier, n8n — each with 8 detailed steps + tip + common mistake (step-by-step for non-technical users)
  - Payload preview block with copy-button
  - Compatible tools strip: Make.com, Zapier, n8n, HubSpot, Pipedrive, monday.com, SendGrid, Slack, WhatsApp Business
  - "Need help?" section at bottom
- Bilingual (He/En), full RTL support, `GlobalFooter` included
- CSS keyframes namespaced `int-*`
- **Layout rule:** how-it-works steps must always use vertical stacked layout — horizontal 3-column grids clip long Hebrew text mid-sentence
...
| Stripe event | Action |
|---|---|
| `checkout.session.completed` | Set `plan_tier` (from price ID) + `billing_status: 'active'` + `subscription_period_end` + `subscription_cancel_at_period_end` + save `stripe_customer_id`; tag subscription with `supabase_user_id` metadata |
| `customer.subscription.updated` | Update `plan_tier` + `subscription_period_end` + `subscription_cancel_at_period_end` from updated subscription |
| `customer.subscription.deleted` | Set `plan_tier: 'free'` + `billing_status: 'canceled'` + clear `subscription_period_end` |
| `invoice.payment_failed` | Set `billing_status: 'past_due'` — triggers dunning UI |
| `invoice.payment_succeeded` | Set `billing_status: 'active'` + refresh `subscription_period_end` + `subscription_cancel_at_period_end` — resolves dunning |
...
subscription_period_end:            string   // Unix timestamp string e.g. "1746000000" — read by useSubscriptionPeriodEnd()
subscription_cancel_at_period_end:  'true' | 'false'  // read by useCancelAtPeriodEnd()
```
...
**"חיוב ומנוי / Billing & Subscription" menu item** — visible to ALL users (free, pro, unlimited). Navigates to `/billing` via `navigate('/billing')`. Color follows tier: indigo for Pro, gold for Premium, muted white for Free. This replaced the old "Manage Subscription" direct-to-Stripe-portal item (Sprint 47.4).
...
Free tier sees a **full-page paywall** (not just a blur overlay on one card). Layout: how-it-works section (stacked, not horizontal — prevents text truncation) + use-cases grid + lock gate with upgrade CTA that opens `UpgradeModal`. Paid users see the full webhook config page with step-by-step platform guides.
...
|---|---|---|
| `useTier()` | `'free' \| 'pro' \| 'unlimited'` | `user_metadata.plan_tier` |
| `useBillingStatus()` | `'active' \| 'past_due' \| 'canceled' \| null` | `user_metadata.billing_status` |
| `useSubscriptionPeriodEnd()` | `Date \| null` | `user_metadata.subscription_period_end` (Unix timestamp string) |
| `useCancelAtPeriodEnd()` | `boolean` | `user_metadata.subscription_cancel_at_period_end` ('true'/'false' string) |

### Navbar entry point

The "חיוב ומנוי / Billing & Subscription" item in the `ProtectedLayout` dropdown navigates to `/billing` via `navigate('/billing')` for **all users** (free, pro, unlimited). It does NOT link directly to the Stripe portal. Color: gold for Premium, indigo for Pro, muted white for Free.
...
- **Do not link "Billing & Subscription" directly to `STRIPE_CUSTOMER_PORTAL` in the navbar** — the item navigates to `/billing` (in-app page) for all users. Sprint 47.4 replaced the direct Stripe portal link with the internal billing page. Free users see upgrade options; paid users see their plan details and a portal link from within the page.
